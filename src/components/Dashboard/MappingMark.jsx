...
/* borrowed ellipsis: the seal a breath before it exists, at estate     */
/* scale. A run of hex cells scrambles faintly in the seal's own voice  */
/* while a read head sweeps left→right; cells under the head hold       */
/* steady and lift to WORKING ink (being read), then fall back to       */
/* scramble as it passes. The run never deepens to true ink — that      */
/* value is earned only when a real anchor locks (SealAnchor). A sweep  */
/* that releases is the honest physics for unknown duration: it reads   */
/* as continuous reading, never as progress toward a completion the     */
/* backend hasn't promised. Imperative on one rAF clock, like the seal: */
/* zero React re-renders mid-sweep.                                     */
/* ------------------------------------------------------------------ */
...
const MAPPING_SWEEP_MS = 3200; /* one full left→right read, then the next begins */
...
export default function MappingMark() {
  ...
  useEffect(() => {
    ...
    const frame = (now) => {
      if (!alive) return;
      /* Hold the clock while the tab is hidden — the sweep never runs unseen. */
      if (document.hidden) {
      ...
      if (flick) last = now;
      /* The head travels from before the first cell to past the last, so each
         pass has a natural lead-in and release — never a hard wrap. */
      const t = ((now - start) % MAPPING_SWEEP_MS) / MAPPING_SWEEP_MS;
